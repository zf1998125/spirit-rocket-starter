...
阿里云RocketMQ SDK
Starter，使用方法与类的方式收发消息。[示例代码仓库](https://gitee.com/zztech2020/sinzetech-rocket-example)
...
数据发送类型大致支持有两种，分别为对象类型与`Byte[]`字节数组类型，对象类型会使用`FastJson`转换为`Byte[]`后再发送给RocketMQ，
...
- `SendHelper.processor()`为可配置的执行器，可多配置，如自定义陪之后会按照配置的顺讯依次执行配置的执行器。 同时，默认配置为空，
  配置为空时会走全局的默认配置，全局默认配置位置为com.sinzetech.rocket.base.config.RocketProperties.defaultSendHelper,
  该标志位在引用Rocket
...
使用默认的实现包时需要在数据库新增[`base_rocket_send_log`](../sinzetech-starter-rocket-helper/doc/db/base_rocket_send_log.sql)
表。
...
- 在仅使用幂等功能的情况时，
  需要在数据库新增[`base_rocket_idempotent_log`幂等表](../sinzetech-starter-rocket-helper/doc/db/base_rocket_idempotent_log.sql)
  表。
- 在仅使用消费消息记录功能的情况时，
